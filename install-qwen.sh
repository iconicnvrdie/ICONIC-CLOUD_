...
#
# Usage:
#   sudo bash qwen-filter-install.sh [interface]                     -> standalone (panel + dashboard)
#   sudo bash qwen-filter-install.sh --join --panel URL --token TOK  -> link this VPS to a central panel as a node
...
#   --join      flag    register this VPS as a node of a central panel
#   --panel <url>       central panel base URL (http://panEl-ip:9090)
#   --token <tok>       one-time node token from the panel "+" button
set -euo pipefail
...
/usr/local/lib/matrix-shield/matrix-shield-node.py|matrix-shield-node.py|MSX_NODE
...
if [ -n "$JOIN" ]; then
  if [ -z "$PANEL_URL" ] || [ -z "$NODE_TOKEN" ]; then
    die "--join requires --panel URL and --token TOKEN (get both from the panel "+" button)"
  fi
  NODE_CONF="$ROOT/etc/matrix-shield/node.conf"
  mkdir -p "$(dirname "$NODE_CONF")"
  cat > "$NODE_CONF" <<EOF
# Matrix Shield node — auto-generated by installer. Points this VPS at a central panel.
PANEL=$PANEL_URL
TOKEN=$NODE_TOKEN
EOF
  log "Wrote node config: $NODE_CONF"

  NODE_UNIT="$UNIT_DIR/matrix-shield-node.service"
  cat > "$NODE_UNIT" <<EOF
[Unit]
Description=Matrix Shield Node — reports status to central panel
After=$BIN_NAME.service

[Service]
Type=simple
ExecStart=/usr/bin/python3 /usr/local/lib/matrix-shield/matrix-shield-node.py
Restart=always
RestartSec=5
...
WantedBy=multi-user.target
EOF
  log "Wrote systemd unit: $NODE_UNIT"
  DASH_UNIT=""
else
...
Environment=MS_INSTALL_FILE=/var/www/qwen/qwen-filter-install.sh
...
if [ -n "$JOIN" ]; then
  systemctl enable matrix-shield-node.service >/dev/null 2>&1 || true
  systemctl restart matrix-shield-node.service || true
  sleep 2
  log "=== NODE LINKED TO PANEL: $PANEL_URL ==="
else
...
if [ -n "$JOIN" ]; then
  log "Node   : linked to panel $PANEL_URL (token $NODE_TOKEN)"
  log "Check  : open the panel web UI to see this VPS appear as ONLINE"
else
